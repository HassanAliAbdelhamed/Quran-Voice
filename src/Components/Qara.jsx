...
function Qaraa() {
    ...
    return (
        <>
            <Box sx={{ display: 'flex', alignItems: 'center', padding: '40px', background: 'linear-gradient(to bottom, #434343 ,#222222)', margin: '-80px -30px 0px' }}>
                <Avatar alt="Cindy Baker" sx={{ width: '100px', height: "100px", marginRight: "30px" }} src={quraa[id].img} />
                ...
                <ol>
                    {swar.map(swara => (
                        <div style={{ color: 'white', padding: '20px 0' ,textAlign:'start' }} key={swara.id}>
                            <li ><Link onClick={()=>{window.scrollTo(0,0)}} to={"/"+id+"/"+convertToThreeDigitString(swara.id)} style={{ textDecoration:"none" , color:"white",padding:'10px ',borderRadius:'14px' , cursor:'pointer', background:"#373535" ,display:'flex',justifyContent:'space-between',alignItems:'center'}}>{swara.title} <ArrowBackIosNewIcon style={{borderRadius:'10px ',background:'#222222' , padding:'3px'}}/></Link></li>
                        </div>
                    ))}
                </ol>
            </menu>
